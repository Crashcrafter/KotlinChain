...
fun ByteArray.publicKeyToAddress(): ByteArray {
    ...
    addressBytes.addAll(checksum.copyOfRange(checksum.size-4, checksum.size).toList())
...
fun validateAddress(address: String): Boolean {
    ...
    val hashedPublicKey = bytes.copyOfRange(0, bytes.size-4)
    val checksum = hashedPublicKey.sha256().sha256()
    val combined = "${hashedPublicKey.toHexString()}${checksum.copyOfRange(checksum.size-4, checksum.size).toHexString()}"
